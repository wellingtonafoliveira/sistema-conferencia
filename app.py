...
query = st.text_input(label, value=default_dt if default_dt else "", key=f"{key_prefix}_query")
...
index = filtered.index(default_dt) if default_dt in filtered else 0
return st.selectbox("DT encontrada", filtered, index=index, key=f"{key_prefix}_select")
...
return 0
return max(int((dt_fim - dt_ini).total_seconds() / 60), 0)
...
def logout():
for k in ["auth_ok", "usuario", "perfil", "selected_dt"]:
...
def normalize_tipo_carga(value):
txt = clean_str(value).upper()
...
"tipo_carga": df["Tipo de carga"].apply(clean_str).apply(normalize_tipo_carga),
...
df = df_raw.copy().dropna(how="all").copy()
...
return load_store().get("conferencias", {})
...
def get_insumos_df():
rows = load_store().get("insumos_cp", [])
return pd.DataFrame(rows) if rows else pd.DataFrame()
...
def get_boc_df():
rows = load_store().get("boc_solicitacoes", [])
return pd.DataFrame(rows) if rows else pd.DataFrame()
...
"tipo_carga": clean_str(df_dt["tipo_carga"].iloc[0]).upper(),
...
def has_insumos_cp(dt):
snapshot = get_dt_snapshot(dt)
if not snapshot:
return False
tipo = normalize_tipo_carga(snapshot["meta"].get("tipo_carga", ""))
if tipo != "CP":
return True
return not get_insumos_by_dt(dt).empty
...
df.at[ultimo_idx, "qtd_conferida"] = int(df.at[ultimo_idx, "qtd_conferida"]) + restante

return apply_statuses(df), True, ""
...
return Paragraph(f"{clean_str(name)}<br/>{clean_str(role)}", style)
...
styles = getSampleStyleSheet()
title_style = ParagraphStyle("PdfTitle", parent=styles["Title"], alignment=TA_CENTER, fontSize=18, leading=22, textColor=colors.HexColor("#1D39C4"), spaceAfter=4)
subtitle_style = ParagraphStyle("PdfSubtitle", parent=styles["Normal"], alignment=TA_CENTER, fontSize=9, textColor=colors.HexColor("#6B7280"), spaceAfter=10)
normal_style = ParagraphStyle("PdfNormal", parent=styles["Normal"], fontSize=9, leading=11)
section_style = ParagraphStyle("SectionTitle", parent=styles["Normal"], fontSize=10, leading=12, textColor=colors.HexColor("#0F172A"))
...
table_data = [["Remessa", "Doc. Ref.", "Material", "Descrição", "Qtd Sol.", "Qtd Conf.", "Status"]]
...
table = Table(table_data, repeatRows=1, colWidths=[26 * mm, 30 * mm, 28 * mm, 104 * mm, 22 * mm, 22 * mm, 24 * mm])
...
tipo_carga = normalize_tipo_carga(meta.get("tipo_carga", ""))
if tipo_carga == "CP":
insumos = get_latest_insumos_cp(dt)
story.append(Spacer(1, 12))
story.append(Paragraph("Insumos da Carga Paletizada (CP)", section_style))
...
str(insumos.get("quadro_sem_ripa", 0)),
str(insumos.get("quadro_com_ripa", 0)),
],
...
boc_table = Table(boc_table_data, repeatRows=1, colWidths=[30 * mm, 25 * mm, 28 * mm, 95 * mm, 18 * mm, 30 * mm])
...
estimado = int(round(total_caixas / media_caixas_min)) if media_caixas_min > 0 else 0
...
combined = combined.drop_duplicates(subset=["dt", "remessa", "doc_referencia", "material"]).reset_index(drop=True)
...
items_df = apply_statuses(snapshot_to_df(snapshot).sort_values(by=["remessa", "material"]).reset_index(drop=True))
...
tempo_estimado = estimate_minutes_by_history(mgmt_hist, total_caixas, metragem_cubica, tipo_carga)

row1 = st.columns(4)
with row1[0]: info_card("Cliente", meta.get("cliente", ""))
with row1[1]: info_card("Transportadora", meta.get("transportadora", ""))
with row1[2]: info_card("Status DT", meta.get("status_dt", "PENDENTE"))
with row1[3]: info_card("Remessas na DT", meta.get("qtd_remessas", 0))

row2 = st.columns(4)
with row2[0]: info_card("SKU únicos", qtd_skus_unicos)
with row2[1]: info_card("Total de caixas", total_caixas)
with row2[2]: info_card("Data agenda", meta.get("data_agenda", ""))
with row2[3]: info_card("Hora agenda", meta.get("hora_agenda", ""))

row3 = st.columns(4)
with row3[0]: info_card("Perfil de carregamento", meta.get("perfil_carregamento", ""))
with row3[1]: info_card("Tipo de carga", tipo_carga)
with row3[2]: info_card("Metragem cúbica", f"{metragem_cubica:.3f} m³")
with row3[3]: info_card("Previsão de conferência", f"{tempo_estimado} min")
...
s1, s2 = st.columns(2)
assinatura_conferente = s1.text_input("Nome para assinatura do conferente", value=meta.get("assinatura_conferente", meta.get("conferente", "")), key=f"assinatura_conf_{dt}")
assinatura_lider = s2.text_input("Nome para assinatura da liderança / responsável", value=meta.get("assinatura_lider", ""), key=f"assinatura_lider_{dt}")
...
st.success(
f"Lançamento realizado para SKU {sku_digitado} - {desc_map.get(sku_digitado, '')}. "
...
editor_df = items_df[["remessa", "doc_referencia", "material", "descricao", "qtd_solicitada", "qtd_conferida", "status_item"]].copy()
...
st.dataframe(boc_df.sort_values("data_hora", ascending=False), use_container_width=True, hide_index=True)
...
st.download_button("Baixar último PDF", data=st.session_state["last_pdf_bytes"], file_name=st.session_state["last_pdf_name"], mime="application/pdf", key=f"download_pdf_{dt}")
...
elif tipo_carga == "CP" and not insumos_ok:
st.error("Para carga paletizada (CP), é obrigatório lançar os insumos antes de finalizar a conferência.")
else:
finalize_dt(dt, "FINALIZADO", conferente, turno, assinatura_conferente, assinatura_lider)
...
if tipo_carga == "CP" and not insumos_ok:
st.error("Para carga paletizada (CP), é obrigatório lançar os insumos antes de finalizar com divergência.")
else:
finalize_dt(dt, "DIVERGENTE", conferente, turno, assinatura_conferente, assinatura_lider)
snapshot_final = get_dt_snapshot(dt)
pdf_bytes = generate_pdf_bytes(snapshot_final)
file_name = f"espelho_dt_{dt}_{now_sp_file()}.pdf"
st.session_state["last_pdf_bytes"] = pdf_bytes
...
set_selected_dt(dt)
snapshot = get_dt_snapshot(dt)
tipo_carga = normalize_tipo_carga(snapshot["meta"].get("tipo_carga", ""))

if tipo_carga != "CP":
st.warning("Esta DT não é do tipo CP. O lançamento de insumos é destinado à carga paletizada.")
else:
st.success("DT CP identificada. Os insumos lançados aqui serão obrigatórios para a finalização e sairão no PDF.")

ult = get_latest_insumos_cp(dt)

col1, col2, col3, col4 = st.columns(4)
palete = col1.number_input("Palete", min_value=0, step=1, value=int(ult.get("palete", 0) or 0), key="insumo_palete")
chapa = col2.number_input("Chapa", min_value=0, step=1, value=int(ult.get("chapa", 0) or 0), key="insumo_chapa")
...
hist = get_insumos_df()
idx = hist.index[hist["dt"].astype(str) == str(dt)].tolist() if not hist.empty else []
...
hist = pd.concat([get_insumos_df(), pd.DataFrame([novo])], ignore_index=True)
save_insumos_df(hist)
...
st.info("Sem lançamentos de insumos para esta DT.")
...
itens_remessa["item_label"] = itens_remessa["material"].astype(str) + " - " + itens_remessa["descricao"].astype(str)
...
hist = pd.concat([get_boc_df(), pd.DataFrame([novo])], ignore_index=True)
save_boc_df(hist)
...
escolha = st.selectbox("Selecione o BOC para excluir", hist_show["label_exclusao"].tolist(), key="boc_delete_select")
...
row_sel = hist_show.loc[hist_show["label_exclusao"] == escolha].iloc[0]
...
save_boc_df(full_hist[~mask].copy())
...
st.plotly_chart(px.pie(status_counts, names="Status", values="Quantidade", title="Status das DTs"), use_container_width=True)

with chart_col2:
st.plotly_chart(
px.bar(
mgmt.groupby("Tipo de carga", dropna=False).size().reset_index(name="Quantidade"),
x="Tipo de carga", y="Quantidade", title="DTs por Tipo de Carga"
),
use_container_width=True
)
...
st.plotly_chart(
px.bar(conf_df.groupby("Conferente", dropna=False).size().reset_index(name="DTs"),
x="Conferente", y="DTs", title="DTs por Conferente"),
use_container_width=True
)

with chart_col4:
st.plotly_chart(
px.bar(mgmt.sort_values("Total caixas", ascending=False).head(10),
x="DT", y="Total caixas", title="Top 10 DTs por Total de Caixas"),
use_container_width=True
)
...
st.dataframe(
ranking[["Posição", "Conferente", "DTs", "DTs_Finalizadas", "DTs_Divergentes", "Total_Caixas", "Tempo Total", "Produtividade", "Total_Divergencias"]],
use_container_width=True,
hide_index=True,
)
else:
...
def page_faturista():
show_logo_main()
st.title("Painel do Faturista")

if get_base_vl06_df().empty:
st.info("Sem base carregada.")
return

mgmt = build_management_df()
conferidas = mgmt[mgmt["Status DT"].isin(["FINALIZADO", "DIVERGENTE"])].copy()

if conferidas.empty:
st.info("Não há DTs conferidas até o momento.")
return

dt_list = conferidas["DT"].astype(str).tolist()
default_dt = get_selected_dt()
query = st.text_input("Pesquisar DT conferida", value=default_dt if default_dt else "", key="fat_query")
filtered = [d for d in dt_list if query.strip() in d] if query.strip() else dt_list
...
index = filtered.index(default_dt) if default_dt in filtered else 0
dt = st.selectbox("DT conferida encontrada", filtered, index=index, key="fat_select")
set_selected_dt(dt)
...
row1 = st.columns(4)
with row1[0]: info_card("DT", dt)
with row1[1]: info_card("Status", meta.get("status_dt", ""))
with row1[2]: info_card("Tipo de carga", meta.get("tipo_carga", ""))
with row1[3]: info_card("Conferente", meta.get("conferente", ""))

row2 = st.columns(4)
with row2[0]: info_card("Cliente", meta.get("cliente", ""))
with row2[1]: info_card("Transportadora", meta.get("transportadora", ""))
with row2[2]: info_card("Data agenda", meta.get("data_agenda", ""))
with row2[3]: info_card("Hora agenda", meta.get("hora_agenda", ""))
...
st.subheader("Itens da DT")
st.dataframe(
items_df[["remessa", "doc_referencia", "material", "descricao", "qtd_solicitada", "qtd_conferida", "status_item"]],
use_container_width=True,
hide_index=True,
)

st.subheader("Insumos CP")
insumos = get_insumos_by_dt(dt)
if not insumos.empty:
st.dataframe(insumos.sort_values("data_hora", ascending=False), use_container_width=True, hide_index=True)
else:
st.info("Sem insumos para esta DT.")

st.subheader("BOC")
boc = get_boc_by_dt(dt)
if not boc.empty:
st.dataframe(boc.sort_values("data_hora", ascending=False), use_container_width=True, hide_index=True)
else:
st.info("Sem BOC para esta DT.")

pdf_url = meta.get("pdf_url", "")
if pdf_url:
col1, col2 = st.columns(2)
with col1:
st.link_button("Abrir PDF online", pdf_url, use_container_width=True)
with col2:
...
st.info("PDF ainda não disponível para esta DT.")


def page_coletor():
show_logo_main()
st.title("Modo Coletor")
st.caption("Tela operacional simplificada para lançamento rápido")

if get_base_vl06_df().empty:
st.info("Sem base carregada.")
return

dt = dt_search_select("Pesquisar DT", key_prefix="coletor_dt")
if not dt:
return
...
if dt_locked(snapshot):
st.error("Esta DT está finalizada e bloqueada para coleta.")
return

meta = snapshot["meta"]
items_df = apply_statuses(snapshot_to_df(snapshot).sort_values(by=["material", "remessa"]).reset_index(drop=True))

colh1, colh2, colh3, colh4 = st.columns(4)
with colh1: info_card("DT", dt)
with colh2: info_card("Status", meta.get("status_dt", ""))
with colh3: info_card("Tipo de carga", meta.get("tipo_carga", ""))
with colh4: info_card("Conferente", meta.get("conferente", ""))

st.subheader("Lançamento rápido")
l1, l2, l3 = st.columns([3, 1, 1])
sku = l1.text_input("Bipar SKU", key="coletor_sku")
qtd = l2.number_input("Qtd", min_value=1, step=1, key="coletor_qtd")
confirmar = l3.button("Baixar", use_container_width=True)

if confirmar:
if not clean_id(sku):
st.warning("Informe o SKU.")
else:
novo_df, ok, msg = lancar_quantidade_sku(items_df, clean_id(sku), int(qtd))
if ok:
update_snapshot_items(dt, novo_df)
...
st.subheader("Visão simplificada da DT")
view = items_df[["material", "descricao", "qtd_solicitada", "qtd_conferida", "status_item"]].copy()
...
c1.metric("Itens OK", ok_count)
c2.metric("Itens divergentes", div_count)
c3.metric("Itens pendentes", pen_count)
...
page_gestao()
elif section == "Faturista":
page_faturista()
elif section == "Coletor":
page_coletor()
